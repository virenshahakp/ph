...
with windowed_distincts as (
  select
    publica_bid_won_dyn_source.partition_date::date
    , publica_bid_won_dyn_source.partition_hour
    , dim_demand_partner_map.demand_partner
    , publica_bid_won_dyn_source.fill_type_classification
    , publica_bid_won_dyn_source.pod_owner
    , publica_bid_won_dyn_source.content_network                                                          as network
    , publica_bid_won_dyn_source.content_channel                                                          as channel
    , date_add('hour', publica_bid_won_dyn_source.partition_hour::int, publica_bid_won_dyn_source.partition_date::timestamp) as utc_ts
    , right(publica_bid_won_dyn_source.targeting_device, length(publica_bid_won_dyn_source.targeting_device) - 23)           as platform
    , md5(
      publica_bid_won_dyn_source.partition_date
      || publica_bid_won_dyn_source.partition_hour
      || publica_bid_won_dyn_source.utc_ts
      || dim_demand_partner_map.demand_partner
      || publica_bid_won_dyn_source.fill_type_classification
      || publica_bid_won_dyn_source.pod_owner
      || publica_bid_won_dyn_source.platform
      || publica_bid_won_dyn_source.network
      || publica_bid_won_dyn_source.channel
    )                                                                                  as md5_hash
    ...
    , dense_rank() over (
      partition by
        ...
        , publica_bid_won_dyn_source.network
        , publica_bid_won_dyn_source.channel
      order by publica_bid_won_dyn_source.endpoint_uuid
...
, publica_bid_won_dyn_source as (
  select
    ...
    , count(1) as publica_bid_won_dyn_source__count
...
select
  ...
  , publica_bid_won_dyn_source.publica_bid_won_dyn_source__count
  , opportunity_and_pods.md5_hash
from opportunity_and_pods
join publica_bid_won_dyn_source on opportunity_and_pods.md5_hash = publica_bid_won_dyn_source.md5_hash
