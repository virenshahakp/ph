...
select
  ...
  , count(
    distinct
    bidder_impression.adbreak_id
    || ':'
    || bidder_impression.session_id
  )                                                           as pod_instance_id_count
  , count(distinct bidder_impression.endpoint_uuid)           as endpoint_uuid_count
  , count(distinct bidder_impression.vmap_uuid)               as vmap_uuid_count
  , sum(coalesce(bidder_impression.ad_duration, 0))           as ad_fill
  , count(1)                                                  as impression_count
  , count(
    distinct bidder_impression.ad_duration
  )                                                           as distinct_ad_duration_count
