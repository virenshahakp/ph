...
select
  ...
  , count(
    distinct bid_error.adbreak_id
    || ':'
    || bid_error.session_id
  )                                                   as pod_instance_id_count
  , count(distinct bid_error.endpoint_uuid)           as endpoint_uuid_count
  ...
  , max(bid_error.bidresponse_cpm)                    as max_cpm
...
from bid_error
left join demand_partner_map on bid_error.bidrequest_bids_headerbidder_id = demand_partner_map.bidder_id
left join publica_platform_map on bid_error.bidrequest_site_id = publica_platform_map.bidrequest_site_id
where bid_error.partition_date between '{{ start_date }}' and '{{ end_date }}'
