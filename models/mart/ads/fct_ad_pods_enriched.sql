...
with pods as (
  select *
  from {{ ref('tbl_vast_ads_to_pods_agg') }} as tbl_vast_ads_to_pods_agg
  where tbl_vast_ads_to_pods_agg.partition_date_hour between
...
, opportunity as (
  select *
  from {{ ref('tbl_dim_opportunity_classifications') }}
  where tbl_dim_opportunity_classifications.partition_date_hour between
    date_add('hour', 0::int, '{{ start_date }}')
    and date_add('hour', 23::int, '{{ end_date }}')
    {% if channel != "" %} 
      and channel = '{{ channel }}' 
