...
-- was "hourly" tagged, excluding while working through debugging

-- Pull in default vars FROM dbt_project.yml
{% set partition_size = 'hour' %}
...
%}
    ...
    {% set max_partition_date%}
        {{ max_time(field_name = "ended_at", datepart = partition_size, tbl = "") }}
    {% endset %}
...
SELECT
    ...
    , DATE_TRUNC('hour', ended_at) AS ended_at
    , platform
    , app_version
    , as_number
    , as_name
...
WHERE 1=1
    ...
    */
    ...
    AND DATEDIFF(minute, ended_at::DATETIME, SYSDATE) >= 75
    ...
    %}
        ...
        */
        ...
        AND ended_at::DATETIME >= {{dbt.dateadd(
