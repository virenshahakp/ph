...
, ios_open as (
  ...
  from {{ ref('ios_prod_application_opened_stage') }}
  where
...
, time_diff as (
  ...
  select
    ...
    , ios_open.open_time
    , datediff('seconds', ios_open.open_time, stream_starts.stream_start_at) as time_diff_sec
  from ios_open
  join stream_starts
    on ios_open.user_id = stream_starts.user_id
      and ios_open.open_date = stream_starts.stream_start_date
  where -- Ensure that we only consider the first 10 mins after application opened
    datediff('seconds', ios_open.open_time, stream_starts.stream_start_at) > 1
    and datediff('seconds', ios_open.open_time, stream_starts.stream_start_at) < 600
  order by
    ios_open.user_id
    , stream_starts.stream_start_at
