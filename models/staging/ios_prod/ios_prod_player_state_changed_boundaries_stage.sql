...
playback_sessions_to_process as (
  ...
  from {{ ref('ios_prod_player_state_changed_stage') }}
  {%- if is_incremental() %}
...
, events_to_process as (
  ...
  select
    ios_prod_player_state_changed_stage.event_id
    , ios_prod_player_state_changed_stage.user_id
    , ios_prod_player_state_changed_stage.playback_session_id
    , ios_prod_player_state_changed_stage.received_at
    , ios_prod_player_state_changed_stage."timestamp"
    , ios_prod_player_state_changed_stage.delay
    , ios_prod_player_state_changed_stage.position_start
    , ios_prod_player_state_changed_stage.position_stop
    , ios_prod_player_state_changed_stage.hashed_session_id
    , ios_prod_player_state_changed_stage.context_ip
    , ios_prod_player_state_changed_stage.action
    , ios_prod_player_state_changed_stage.bitrate
    , ios_prod_player_state_changed_stage.loaded_at
    /*
    ...
    */
    , case
      when regexp_count(ios_prod_player_state_changed_stage.decoded_asset_id, ':') > 1
        then f_base64encode(
            split_part(
              ios_prod_player_state_changed_stage.decoded_asset_id, ':', 1
            ) || ':'
            || split_part(
              ios_prod_player_state_changed_stage.decoded_asset_id, ':', 2
            )
          )
      else ios_prod_player_state_changed_stage.asset_id
    end                                                                                                      as asset_id
  from {{ ref('ios_prod_player_state_changed_stage') }}
