...
, android_open as (
  ...
  from {{ ref('androidtv_prod_application_opened_stage') }}
  where
...
, time_diff as (
  ...
  select
    android_open.user_id
    , stream_starts.platform
    , android_open.open_date
    , stream_starts.stream_start_at
    , android_open.open_time
    , datediff('seconds', android_open.open_time, stream_starts.stream_start_at) as time_diff_sec
  from android_open
  join stream_starts
    on android_open.user_id = stream_starts.user_id
      and android_open.open_date = stream_starts.stream_start_date
  where -- Ensure that we only consider the first 10 mins after application opened
    datediff('seconds', android_open.open_time, stream_starts.stream_start_at) > 1
    and datediff('seconds', android_open.open_time, stream_starts.stream_start_at) < 600
  order by
    android_open.user_id
    , stream_starts.stream_start_at
