...
playback_sessions_to_process as (
  ...
  from {{ ref('fire_tv_prod_player_state_changed_stage') }}
  {%- if is_incremental() %}
...
, events_to_process as (
  ...
  select
    fire_tv_prod_player_state_changed_stage.event_id
    , fire_tv_prod_player_state_changed_stage.user_id
    , fire_tv_prod_player_state_changed_stage.playback_session_id
    , fire_tv_prod_player_state_changed_stage.received_at
    , fire_tv_prod_player_state_changed_stage."timestamp"
    , fire_tv_prod_player_state_changed_stage.delay
    , fire_tv_prod_player_state_changed_stage.position_start
    , fire_tv_prod_player_state_changed_stage.position_stop
    , fire_tv_prod_player_state_changed_stage.hashed_session_id
    , fire_tv_prod_player_state_changed_stage.context_ip
    , fire_tv_prod_player_state_changed_stage.action
    , fire_tv_prod_player_state_changed_stage.bitrate
    , fire_tv_prod_player_state_changed_stage.loaded_at
    /*
    ...
    */
    , case
      when regexp_count(fire_tv_prod_player_state_changed_stage.decoded_asset_id, ':') > 1
        then f_base64encode(
            split_part(
              fire_tv_prod_player_state_changed_stage.decoded_asset_id, ':', 1
            ) || ':'
            || split_part(
              fire_tv_prod_player_state_changed_stage.decoded_asset_id, ':', 2
            )
          )
      else fire_tv_prod_player_state_changed_stage.asset_id
    end                                                                                                      as asset_id
  from {{ ref('fire_tv_prod_player_state_changed_stage') }}
