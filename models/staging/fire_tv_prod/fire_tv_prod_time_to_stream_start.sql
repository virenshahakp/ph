...
, firetv_open as (
  ...
  from {{ ref('fire_tv_prod_application_opened_stage') }}
  where
...
, time_diff as (
  ...
  select
    ...
    , firetv_open.open_date
    , stream_starts.stream_start_at
    , firetv_open.open_time
    , datediff('seconds', firetv_open.open_time, stream_starts.stream_start_at) as time_diff_sec
  from firetv_open
  join stream_starts
    on firetv_open.user_id = stream_starts.user_id
      and firetv_open.open_date = stream_starts.stream_start_date
  where -- Ensure that we only consider the first 10 mins after application opened
    datediff('seconds', firetv_open.open_time, stream_starts.stream_start_at) > 1
    and datediff('seconds', firetv_open.open_time, stream_starts.stream_start_at) < 600
  order by
    firetv_open.user_id
    , stream_starts.stream_start_at
