...
, roku_open as (
  ...
  from {{ ref('roku_prod_application_opened_stage') }}
  where
...
, time_diff as (
  ...
  select
    ...
    , roku_open.open_time
    , datediff('seconds', roku_open.open_time, stream_starts.stream_start_at) as time_diff_sec
  from
    roku_open
  join stream_starts
    on roku_open.user_id = stream_starts.user_id
      and roku_open.open_date = stream_starts.stream_start_date
  where  -- Ensure that we only consider the first 10 mins after application opened
    datediff('seconds', roku_open.open_time, stream_starts.stream_start_at) > 1
    and datediff('seconds', roku_open.open_time, stream_starts.stream_start_at) < 600
  order by
    roku_open.user_id
    , stream_starts.stream_start_at
