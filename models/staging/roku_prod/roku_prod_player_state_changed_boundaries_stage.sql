...
playback_sessions_to_process as (
  ...
  from {{ ref('roku_prod_player_state_changed_stage') }}
  {%- if is_incremental() %}
    where dbt_processed_at > {{ max_processed_at }}
  {%- endif %}
  /* when deploying and building history this is needed to batch records */
  {#
    {%- if is_incremental() %} 
    where not in (
...
, events_to_process as (
  ...
  select
    roku_prod_player_state_changed_stage.event_id
    , roku_prod_player_state_changed_stage.user_id
    , roku_prod_player_state_changed_stage.playback_session_id
    , roku_prod_player_state_changed_stage.received_at
    , roku_prod_player_state_changed_stage."timestamp"
    , roku_prod_player_state_changed_stage.delay
    , roku_prod_player_state_changed_stage.position_start
    , roku_prod_player_state_changed_stage.position_stop
    , roku_prod_player_state_changed_stage.hashed_session_id
    , roku_prod_player_state_changed_stage.context_ip
    , roku_prod_player_state_changed_stage.action
    , roku_prod_player_state_changed_stage.bitrate
    , roku_prod_player_state_changed_stage.loaded_at
    /*
    ...
    */
    , case
      when regexp_count(roku_prod_player_state_changed_stage.decoded_asset_id, ':') > 1
        then f_base64encode(
            split_part(
              roku_prod_player_state_changed_stage.decoded_asset_id, ':', 1
            ) || ':'
            || split_part(
              roku_prod_player_state_changed_stage.decoded_asset_id, ':', 2
            )
          )
      else roku_prod_player_state_changed_stage.asset_id
    end                                                                                                      as asset_id
  from {{ ref('roku_prod_player_state_changed_stage') }}
