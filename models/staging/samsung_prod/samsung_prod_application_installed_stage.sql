...
app_installs as (
  ...
  select * from {{ ref('samsung_prod_application_installed_source') }}
...
select
  ...
  , case when coalesce(trim(appsflyer.attributed_touch_type), '') != '' then 1
    else app_installs.priority
  end                      as priority

from app_installs
left join appsflyer on (
  app_installs.context_device_advertising_id = appsflyer.context_device_advertising_id
  and coalesce(trim(appsflyer.attributed_touch_type), '') != ''
)
