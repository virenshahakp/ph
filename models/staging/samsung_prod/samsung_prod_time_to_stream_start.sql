...
, samsung_open as (
  ...
  from {{ ref('samsung_prod_application_opened_stage') }}
  where
...
, time_diff as (
  ...
  select
    samsung_open.user_id
    , stream_starts.platform
    , samsung_open.open_date
    , stream_starts.stream_start_at
    , samsung_open.open_time
    , datediff('seconds', samsung_open.open_time, stream_starts.stream_start_at) as time_diff_sec
  from samsung_open
  join stream_starts
    on samsung_open.user_id = stream_starts.user_id
      and samsung_open.open_date = stream_starts.stream_start_date
  where -- Ensure that we only consider the first 10 mins after application opened
    datediff('seconds', samsung_open.open_time, stream_starts.stream_start_at) > 1
    and datediff('seconds', samsung_open.open_time, stream_starts.stream_start_at) < 600
  order by
    samsung_open.user_id
    , stream_starts.stream_start_at
