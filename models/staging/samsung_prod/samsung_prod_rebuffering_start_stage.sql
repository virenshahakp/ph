...
rebuffering_start as (
  ...
  select * from {{ ref('samsung_prod_rebuffering_start_source') }}
...
left join agents on (rebuffering_start.context_user_agent_id = agents.context_user_agent_id)
{% if is_incremental() %}
  where rebuffering_start.loaded_at > {{ max_loaded_at }}
{% endif %}
