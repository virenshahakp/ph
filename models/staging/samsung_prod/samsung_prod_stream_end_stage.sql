...
stream_end as (
  ...
  select * from {{ ref('samsung_prod_stream_end_source') }}
...
from stream_end
left join agents on (stream_end.context_user_agent_id = agents.context_user_agent_id)
{% if is_incremental() %}
  where stream_end.loaded_at > {{ max_loaded_at }}
{% endif %}
