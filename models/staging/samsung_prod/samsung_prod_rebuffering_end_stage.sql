...
rebuffering_end as (
  ...
  select * from {{ ref('samsung_prod_rebuffering_end_source') }}
...
left join agents on (rebuffering_end.context_user_agent_id = agents.context_user_agent_id)
{% if is_incremental() %}
  where rebuffering_end.loaded_at > {{ max_loaded_at }}
{% endif %}