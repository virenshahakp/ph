...
playback_sessions_to_process as (
  ...
  from {{ ref('chromecast_prod_player_state_changed_stage') }}
  {%- if is_incremental() %}
...
, events_to_process as (
  ...
  select
    chromecast_prod_player_state_changed_stage.event_id
    , chromecast_prod_player_state_changed_stage.user_id
    , chromecast_prod_player_state_changed_stage.playback_session_id
    , chromecast_prod_player_state_changed_stage.received_at
    , chromecast_prod_player_state_changed_stage."timestamp"
    , chromecast_prod_player_state_changed_stage.delay
    , chromecast_prod_player_state_changed_stage.position_start
    , chromecast_prod_player_state_changed_stage.position_stop
    , chromecast_prod_player_state_changed_stage.hashed_session_id
    , chromecast_prod_player_state_changed_stage.context_ip
    , chromecast_prod_player_state_changed_stage.action
    , chromecast_prod_player_state_changed_stage.bitrate
    , chromecast_prod_player_state_changed_stage.loaded_at
    /*
    ...
    2) sequence_number which was added in October 2023.
    ...
    */
    , case
      when regexp_count(chromecast_prod_player_state_changed_stage.decoded_asset_id, ':') > 1
        then f_base64encode(
            split_part(
              chromecast_prod_player_state_changed_stage.decoded_asset_id, ':', 1
            ) || ':'
            || split_part(
              chromecast_prod_player_state_changed_stage.decoded_asset_id, ':', 2
            )
          )
      else chromecast_prod_player_state_changed_stage.asset_id
    end                                                                                                      as asset_id
  from {{ ref('chromecast_prod_player_state_changed_stage') }}
...
, fill_in_watched_range_timestamps as (
  ...
  select
    ...
    , last_value(
      ...
      over (partition by playback_session_id order by coalesce(sequence_number, rn) rows unbounded preceding)
