...
playback_sessions_to_process as (
  ...
  from {{ ref('tvos_prod_player_state_changed_stage') }}
  {%- if is_incremental() %}
...
, events_to_process as (
  ...
  select
    tvos_prod_player_state_changed_stage.event_id
    , tvos_prod_player_state_changed_stage.user_id
    , tvos_prod_player_state_changed_stage.playback_session_id
    , tvos_prod_player_state_changed_stage.received_at
    , tvos_prod_player_state_changed_stage."timestamp"
    , tvos_prod_player_state_changed_stage.delay
    , tvos_prod_player_state_changed_stage.position_start
    , tvos_prod_player_state_changed_stage.position_stop
    , tvos_prod_player_state_changed_stage.hashed_session_id
    , tvos_prod_player_state_changed_stage.context_ip
    , tvos_prod_player_state_changed_stage.action
    , tvos_prod_player_state_changed_stage.bitrate
    , tvos_prod_player_state_changed_stage.loaded_at
    /*
    ...
    */
    , case
      when regexp_count(tvos_prod_player_state_changed_stage.decoded_asset_id, ':') > 1
        then f_base64encode(
            split_part(
              tvos_prod_player_state_changed_stage.decoded_asset_id, ':', 1
            ) || ':'
            || split_part(
              tvos_prod_player_state_changed_stage.decoded_asset_id, ':', 2
            )
          )
      else tvos_prod_player_state_changed_stage.asset_id
    end                                                                                                      as asset_id
  from {{ ref('tvos_prod_player_state_changed_stage') }}
