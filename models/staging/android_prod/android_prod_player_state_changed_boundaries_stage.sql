...
playback_sessions_to_process as (
  ...
  from {{ ref('android_prod_player_state_changed_stage') }}
  {%- if is_incremental() %}
...
, events_to_process as (
  ...
  select
    android_prod_player_state_changed_stage.event_id
    , android_prod_player_state_changed_stage.user_id
    , android_prod_player_state_changed_stage.playback_session_id
    , android_prod_player_state_changed_stage.received_at
    , android_prod_player_state_changed_stage."timestamp"
    , android_prod_player_state_changed_stage.delay
    , android_prod_player_state_changed_stage.position_start
    , android_prod_player_state_changed_stage.position_stop
    , android_prod_player_state_changed_stage.hashed_session_id
    , android_prod_player_state_changed_stage.context_ip
    , android_prod_player_state_changed_stage.action
    , android_prod_player_state_changed_stage.bitrate
    , android_prod_player_state_changed_stage.loaded_at
    /*
    ...
    */
    , case
      when regexp_count(android_prod_player_state_changed_stage.decoded_asset_id, ':') > 1
        then f_base64encode(
            split_part(
              android_prod_player_state_changed_stage.decoded_asset_id, ':', 1
            ) || ':'
            || split_part(
              android_prod_player_state_changed_stage.decoded_asset_id, ':', 2
            )
          )
      else android_prod_player_state_changed_stage.asset_id
    end                                                                                                      as asset_id
  from {{ ref('android_prod_player_state_changed_stage') }}
