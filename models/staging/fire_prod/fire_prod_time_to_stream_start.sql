...
, fire_open as (
  ...
  from {{ ref('fire_prod_application_opened_stage') }}
  where
...
, time_diff as (
  ...
  select
    ...
    , fire_open.open_time
    , datediff('seconds', fire_open.open_time, stream_starts.stream_start_at) as time_diff_sec
  from fire_open
  join stream_starts
    on fire_open.user_id = stream_starts.user_id
      and fire_open.open_date = stream_starts.stream_start_date
  where -- Ensure that we only consider the first 10 mins after application opened
    datediff('seconds', fire_open.open_time, stream_starts.stream_start_at) > 1
    and datediff('seconds', fire_open.open_time, stream_starts.stream_start_at) < 600
  order by
    fire_open.user_id
    , stream_starts.stream_start_at
