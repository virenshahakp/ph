...
playback_sessions_to_process as (
  ...
  from {{ ref('web_prod_player_state_changed_stage') }}
  {%- if is_incremental() %}
...
, events_to_process as (
  ...
  select
    web_prod_player_state_changed_stage.event_id
    , web_prod_player_state_changed_stage.user_id
    , web_prod_player_state_changed_stage.playback_session_id
    , web_prod_player_state_changed_stage.received_at
    , web_prod_player_state_changed_stage."timestamp"
    , web_prod_player_state_changed_stage.delay
    , web_prod_player_state_changed_stage.position_start
    , web_prod_player_state_changed_stage.position_stop
    , web_prod_player_state_changed_stage.hashed_session_id
    , web_prod_player_state_changed_stage.context_ip
    , web_prod_player_state_changed_stage.action
    , web_prod_player_state_changed_stage.bitrate
    , web_prod_player_state_changed_stage.loaded_at
    /*
    ...
    */
    , case
      when regexp_count(web_prod_player_state_changed_stage.decoded_asset_id, ':') > 1
        then f_base64encode(
            split_part(
              web_prod_player_state_changed_stage.decoded_asset_id, ':', 1
            ) || ':'
            || split_part(
              web_prod_player_state_changed_stage.decoded_asset_id, ':', 2
            )
          )
      else web_prod_player_state_changed_stage.asset_id
    end                                                                                                      as asset_id
  from {{ ref('web_prod_player_state_changed_stage') }}
