...
playback_sessions_to_process as (
  ...
  from {{ ref('viziotv_prod_player_state_changed_stage') }}
  {%- if is_incremental() %}
...
, events_to_process as (
  ...
  select
    viziotv_prod_player_state_changed_stage.event_id
    , viziotv_prod_player_state_changed_stage.user_id
    , viziotv_prod_player_state_changed_stage.playback_session_id
    , viziotv_prod_player_state_changed_stage.received_at
    , viziotv_prod_player_state_changed_stage."timestamp"
    , viziotv_prod_player_state_changed_stage.delay
    , viziotv_prod_player_state_changed_stage.position_start
    , viziotv_prod_player_state_changed_stage.position_stop
    , viziotv_prod_player_state_changed_stage.hashed_session_id
    , viziotv_prod_player_state_changed_stage.context_ip
    , viziotv_prod_player_state_changed_stage.action
    , viziotv_prod_player_state_changed_stage.bitrate
    , viziotv_prod_player_state_changed_stage.loaded_at
    /*
    ...
    */
    , case
      when regexp_count(viziotv_prod_player_state_changed_stage.decoded_asset_id, ':') > 1
        then f_base64encode(
            split_part(
              viziotv_prod_player_state_changed_stage.decoded_asset_id, ':', 1
            ) || ':'
            || split_part(
              viziotv_prod_player_state_changed_stage.decoded_asset_id, ':', 2
            )
          )
      else viziotv_prod_player_state_changed_stage.asset_id
    end                                                                                                      as asset_id
  from {{ ref('viziotv_prod_player_state_changed_stage') }}
