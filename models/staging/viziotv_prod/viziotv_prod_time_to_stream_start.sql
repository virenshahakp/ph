...
, viziotv_open as (
  ...
  from {{ ref('viziotv_prod_application_opened_stage') }}
  where
...
, time_diff as (
  ...
  select
    viziotv_open.user_id
    , stream_starts.platform
    , viziotv_open.open_date
    , stream_starts.stream_start_at
    , viziotv_open.open_time
    , datediff('seconds', viziotv_open.open_time, stream_starts.stream_start_at) as time_diff_sec
  from viziotv_open
  join stream_starts
    on viziotv_open.user_id = stream_starts.user_id
      and viziotv_open.open_date = stream_starts.stream_start_date
  where -- Ensure that we only consider the first 10 mins after application opened
    datediff('seconds', viziotv_open.open_time, stream_starts.stream_start_at) > 1
    and datediff('seconds', viziotv_open.open_time, stream_starts.stream_start_at) < 600
  order by
    viziotv_open.user_id
    , stream_starts.stream_start_at
