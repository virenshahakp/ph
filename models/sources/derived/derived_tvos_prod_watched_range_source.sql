...
source as (
  ...
  select * from {{ source('derived', 'tvos_prod_watched_range') }}
...
, renamed as (
  ...
  from source
  -- Prior to Apple analytics version 12 we used watched_seconds event which
  -- we convert to this watched_range event. With version 12 we switched to
  -- player_state_changed event.
  where (environment_analytics_version is null or environment_analytics_version < 12)
